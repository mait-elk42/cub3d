...
/*   Updated: 2024/06/05 10:35:15 by aabouqas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	send_ray_from(t_vector from, double rad, int color)
{
	...
	while (1)
	{
		...
		if (data->maps[from.y / 26][from.x / 26] == '1')
		{
			...
			double ray_angle = (data->player.angle - mth_degtorad(60)) + ((ray_i * mth_degtorad(60)) / 60);
			double res = ray_angle - data->player.angle;
			des *= cos(res);
			return (des);
			// return (line_length);
		}
